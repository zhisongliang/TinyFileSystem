#define SUCCESS 1               // success
#define FILEO_ERR -1            // open file error
#define FILER_ERR -2            // read file error
#define FILEW_ERR -3            // write file error
#define FILWC_ERR -4            // close file error
#define ALREADY_MOUNTED_ERR -5  // there is already a disk mounted
#define MAGICNUM_INVALID_ERR -6 // magic number is invalid
#define NO_MOUNTED_ERR -7       // NO file mounted
#define REACH_FILE_LIMIT_ERR -8 // reach file limit
#define FILE_NOT_FOUND_ERR -9   // file not found