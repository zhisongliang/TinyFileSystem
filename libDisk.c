...
*/
{
	...
	if (lseek(disk, byteOffset, SEEK_SET) == -1) return FILER_ERR;
	if (read(disk, block, BLOCKSIZE) < -1) return FILER_ERR;
	return 0;
}
...
*/ 
{
   ...
   if (lseek(disk, offset, SEEK_SET) == -1) return FILEW_ERR;
   if ((write(disk, block, BLOCKSIZE)) <= 0) return FILEW_ERR;
   return 0;
}
